...
ModbusDevice::ModbusDevice(uint8_t id, RS485 &rs485, double decayRate) : deviceID(id), rs485(rs485), errorDecayRate(decayRate) {}
...
// uint16_t ModbusDevice::crc16(const uint8_t *buffer, uint8_t buffer_length) {
...
int ModbusDevice::readRegisters(uint16_t reg, uint8_t func, uint16_t count, uint16_t *data) {
    ...
    while (true) {
        ...
        if (numRecv < responseLen) { // Haven't gotten a full message yet, get the next char
            continue;
        }
        ...
        if (raw[0] == deviceID && raw[1] == func && raw[2] == count * 2 &&
            ...
            uint16_t crcPack = (raw[responseLen - 2] | (raw[responseLen - 1] << 8));
            uint16_t crcCalc = crc16(raw, responseLen - 2);
...
int ModbusDevice::writeMultiple(uint16_t reg, uint8_t func, uint16_t count, const uint16_t *data) {
    ...
    while (true) {
        ...
        if (numRecv < 8) { // Haven't gotten a full message yet, get the next char
            continue;
        }
